module.exports = {
    name: "mvcha",
    description: "Rename/Remove a channel to category",
    args: true,
    ...
    async execute(msg, args) {
        ...
        if (new_category) {
            ...
            const givenCategory = msg.client.channels.cache.find(
                (category) =>
                    category.name.toLowerCase() === new_category.toLowerCase()
            );
            if (!givenCategory) {
                return msg.channel.send(
                    `❌ There's no category with this name, try again!`
                );
            }

            // rename the channel to the new name
            if (
                channel_id.setParent(givenCategory.id) &&
                channel_id.setName(new_channel_name)
            ) {
                return msg.channel.send(
                    `✅ Channel has been renamed and moved !`
                );
            } else {
                return msg.channel.send(
            ...
            if (channel_id.setName(new_channel_name)) {
                return msg.channel.send(
                    `✅ Channel has been renamed successfully!`
                );
            } else {
                return msg.channel.send(
                    `❌ Something went wrong, please try again!`
