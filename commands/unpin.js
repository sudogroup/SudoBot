module.exports = {
	name: 'unpin',
	description: 'Unpins a message or messages by giving partial contant of the message, or the word ALL to unpin all messages!',
	...
	execute(msg, args) {
		...
		msg.channel.messages.fetchPinned(false).then(pinned_messages => {
			...
			if(args[0] === 'ALL') {
				...
				console.log(`We have ${pinned_messages.size} messages and ${args.length} keywords`);
				pinned_messages.forEach(element => {
					...
					args.forEach(search_word => {
						if(!element.content.toLowerCase().includes(search_word.toLowerCase())) {
			...
			if(unpinned_messages_counter > 0) {
				msg.channel.send(`Unpinned ${unpinned_messages_counter} messages`);
