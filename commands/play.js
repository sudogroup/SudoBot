...
const { musicQueue } = require('../assets/musicQueue.js');
 ...
 function play(guild,song,msg){
    ...
    if (dispatcher) {
        ...
        msg.channel.send(`${msg.author.toString()} playing (${guild,serverQueue.songs[0].title})`)
    }
};
...
module.exports = {
    ...
    async execute(msg, args) {
        ...
        if (!voiceChannel) return msg.channel.send(`${msg.author.tag} You need to be in a voice channel to play music!`);
        const permissions = voiceChannel.permissionsFor(msg.client.user);
        if (!permissions.has('CONNECT')) return msg.channel.send(`I don't have permissions to connect to the voice channel`);
        if (!permissions.has('SPEAK')) return msg.channel.send(`I don't have permissions to speak in the channel`);
